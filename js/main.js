...
function createCrystalHotspotGeometry(
  radius,
  height,
  radialSegments,
  preset = {},
...
function createOrbitalHotspotGeometry(
  ...
  const ringTubularSegments = Math.max(10, Math.floor(radialSegments * 1.8));
  const ringRadialSegments = Math.max(6, Math.floor(radialSegments * 0.6));
...
function createBeaconHotspotGeometry(
  radius,
  height,
  radialSegments,
  preset = {},
  ...
  const tier2Radius = Math.max(
    0.01,
    radius * (preset.tier2RadiusScale ?? 0.68),
  );
...
function buildHotspotGeometryFromLayout() {
  ...
  const domeHeightRatio =
    preset.domeHeightRatio ?? CONFIG.hotspots.dome.heightRatio;
...
function collectArcPointsBetween(a, b, lineCfg) {
  ...
  for (let s = 0; s <= segments; s += 1) {
    ...
    points.push(
      new THREE.Vector3(
        a.x * oneMinus + b.x * t,
        a.y * oneMinus + b.y * t + Math.sin(Math.PI * t) * lineCfg.arcHeight,
        a.z * oneMinus + b.z * t,
      ),
    );
  }
...
function findHotspotIndexById(id) {
  ...
  return hotspotData.findIndex((hs) => String(hs.id).toLowerCase() === needle);
}
...
function updateCanvasLogoPlacement() {
  ...
  const baseZoom = Math.max(0.1, CONFIG.camera.defaultZoom ?? 1);
  const referenceViewHeight = (camera.top - camera.bottom) / baseZoom;
  const logoHeight = Math.max(
    0.01,
    referenceViewHeight * (logoCfg.sizeByViewportHeight ?? 0.11),
  );
...
const mapFitProbe = new THREE.Vector3();
const cameraDefaultZoom = CONFIG.camera.defaultZoom;
const controlsMinZoomBase = CONFIG.controls.minZoom;
const controlsMaxZoomBase = CONFIG.controls.maxZoom;
const mobileTabletMaxShortEdge = CONFIG.camera.mobileTabletMaxShortEdge ?? 1024;
const mobileTabletMaxLongEdge = CONFIG.camera.mobileTabletMaxLongEdge ?? 1366;
const mobileFitPadding = Math.max(
  0.5,
  Math.min(1, CONFIG.camera.mobileFitPadding ?? 0.92),
);
...
function isMobileTabletViewport() {
  ...
  return (
    shortEdge <= mobileTabletMaxShortEdge && longEdge <= mobileTabletMaxLongEdge
  );
}

function getMobileFitZoom() {
  if (!dotPositions.length) return cameraDefaultZoom;
  ...
  dotPositions.forEach((p) => {
    mapFitProbe
      .set(getMapWorldX(p.x), DOT_BASE_Y, -p.y * SPACING)
      .applyMatrix4(camera.matrixWorldInverse);

    if (mapFitProbe.x < minViewX) minViewX = mapFitProbe.x;
    if (mapFitProbe.x > maxViewX) maxViewX = mapFitProbe.x;
    if (mapFitProbe.y < minViewY) minViewY = mapFitProbe.y;
    if (mapFitProbe.y > maxViewY) maxViewY = mapFitProbe.y;
  });

  if (
    !Number.isFinite(minViewX) ||
    !Number.isFinite(maxViewX) ||
    !Number.isFinite(minViewY) ||
    !Number.isFinite(maxViewY)
  ) {
    return cameraDefaultZoom;
  }

  const halfSpanX = Math.max(Math.abs(minViewX), Math.abs(maxViewX));
  const halfSpanY = Math.max(Math.abs(minViewY), Math.abs(maxViewY));
  if (halfSpanX <= 0 || halfSpanY <= 0) return cameraDefaultZoom;

  const aspect = window.innerWidth / window.innerHeight;
  const frustumSize = CONFIG.camera.frustumSize;
  const zoomLimitByWidth = (frustumSize * aspect) / halfSpanX;
  const zoomLimitByHeight = frustumSize / halfSpanY;
  const fitZoom =
    Math.min(zoomLimitByWidth, zoomLimitByHeight) * mobileFitPadding;

  if (!Number.isFinite(fitZoom) || fitZoom <= 0) return cameraDefaultZoom;
  return Math.min(cameraDefaultZoom, fitZoom);
}

function applyResponsiveCameraZoom() {
  const zoom = isMobileTabletViewport()
    ? getMobileFitZoom()
    : cameraDefaultZoom;
  camera.zoom = Math.max(0.1, zoom);
  camera.updateProjectionMatrix();

  controls.minZoom = Math.min(controlsMinZoomBase, camera.zoom);
  controls.maxZoom = Math.max(controlsMaxZoomBase, camera.zoom);
}
...
function buildDotsFromFile(dots) {
  ...
  const haloHeightOffset =
    hotspotHeight * (glowCfg?.heightFactor ?? 0.14) +
    (glowCfg?.yOffset ?? 0.02);
  ...
  Object.entries(grouped).forEach(([type, list]) => {
    ...
    const capMaterial = new THREE.MeshPhysicalMaterial({
      ...
      clearcoat:
        hotspotLayoutPreset.capClearcoat ??
        CONFIG.hotspots.capMaterial.clearcoat,
      clearcoatRoughness:
...
function buildHotspotConnectionLine() {
  if (!isSequentialConnectionEnabled() || hotspotData.length < 2) {
    clearHotspotConnectionLine();
    return;
  }
